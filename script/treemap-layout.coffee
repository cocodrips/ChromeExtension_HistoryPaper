$ ->
  ...
  d3.json("/static/flare.json", (data) =>
    ...
    d3.select(window).on("click", (d) => zoom(root))
    ...
    zoom = (d) ->
      ...
      t.select("rect")
        .attr("width",(d)=> return kx * d.dx  - 1)
        .attr("height",(d)=> return ky * d.dy - 1)

      t.select("text")
        .attr("x",(d)=> return kx * d.dx / 2)
        .attr("y",(d)=> return ky * d.dy / 2)
        .style("opacity", (d)=>return kx * d.dx * extend > d.w ? 1 : 0)
